...
[IMPORTANT]
====
The upgrade process described in this document changes the existing Anypoint Platform APIs, which affects CI/CD pipelines built against the API Manager API and autodiscovery. See link:/release-notes/upgrade#apis-in-api-manager-changes-after-the-upgrade[API Manager Changes] below for more information.
